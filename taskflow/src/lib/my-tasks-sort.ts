...
/** Shape returned by deriveCounts. */
...
/**
 * Derive summary-strip counts from the loaded issues dataset.
 *
 * MYTASK-02: counts are derived purely from the loaded dataset — no additional API calls.
 *
 * @param issues       The flat array of issues (parents + subtasks) from the active query.
 * @param mrAwaitingMeKeys  Issue keys that have an open MR authored by the current user.
 * @param today        Reference date for overdue detection (defaults to now; pass a fixed
 *                     Date in tests for determinism).
 */
export function deriveCounts(
  issues: JiraIssue[],
  mrAwaitingMeKeys: Set<string>,
  today: Date = new Date(),
): MyTaskCounts {
  let toDo = 0;
  let inProgress = 0;
  ...
  for (const issue of issues) {
    ...
    if (cat === 'new') {
      toDo++;
    } else if (cat === 'indeterminate' && !name.includes('review')) {
      inProgress++;
    } else if (cat === 'indeterminate' && name.includes('review')) {
      inReview++;
    } else if (cat === 'done') {
    ...
    // Overdue: past duedate AND not done
    const duedate = issue.fields.duedate as string | null | undefined;
    if (duedate && cat !== 'done') {
      const due = new Date(duedate);
      due.setHours(23, 59, 59, 999);
