...
describe('AIO_STATUS_MAP', () => {
  ...
  it('maps status ID 52 to "inProgress"', () => {
    expect(AIO_STATUS_MAP[52]).toBe('inProgress');
  });

  it('maps status ID 53 to "pass"', () => {
    expect(AIO_STATUS_MAP[53]).toBe('pass');
  });

  it('maps status ID 54 to "fail"', () => {
    expect(AIO_STATUS_MAP[54]).toBe('fail');
  });
