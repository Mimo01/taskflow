...
describe('jira service', () => {
  ...
  describe('EPIC-03: fetchEpicStories', () => {
    ...
    it('returns stories for a given epicKey, excluding subtasks', async () => {
      const stories = [makeStory('PROJ-1'), makeStory('PROJ-2')];
      vi.mocked(mockFetch).mockResolvedValue({
        ok: true,
        status: 200,
      ...
      expect(decodeURIComponent(url)).toContain('"Epic Link" = PROJ-42');
  ...
  describe('fetchAllAssignedHierarchy', () => {
    it('E1: JQL includes sprint scoping to exclude closed sprints', async () => {
      ...
      // Done stories show only from the current (open) sprint; future-sprint and backlog
      // issues are included only when NOT done.
      expect(decodedUrl).toContain(
        '(sprint in futureSprints() OR sprint is EMPTY) AND statusCategory != Done',
      );
    });
