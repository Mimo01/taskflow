...
export function BoardColumn({
  ...
  const cardListClass = [
    ...
    isDisabledForActive ? 'opacity-40 pointer-events-none' : '',
  ]
    .filter(Boolean)
    .join(' ');
      ...
      >
        {groups.length === 0 ? (
          ...
          groups.map((group, idx) => {
            if (group.isBareStory) {
              ...
              return (
                <React.Fragment key={storyIssue.id}>
                  <DraggableCard issue={storyIssue} isSubtask={false} onOpenDetail={onOpenDetail} />
                  {storyError && <p className="text-xs text-destructive px-1">{storyError}</p>}
                </React.Fragment>
              );
            }
            ...
            return (
              <div
                key={headerIssue ? headerIssue.id : `group-${idx}`}
                ...
                {group.cards.map((card) => {
                  ...
                  return (
                    <React.Fragment key={card.id}>
                      <DraggableCard issue={card} isSubtask={true} onOpenDetail={onOpenDetail} />
                      {cardError && <p className="text-xs text-destructive px-1">{cardError}</p>}
                    </React.Fragment>
                  );
                })}
              </div>
