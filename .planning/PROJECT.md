...
The v1.14 Release Management milestone shipped 2026-08-13. Run `/gsd-new-milestone` to open v1.15 (questioning → research → requirements → roadmap).

## Latest Shipped Milestone: v1.14 Release Management (shipped 2026-08-13)

**Goal:** Turn the Releases view from a read-only Jira↔GitLab match into a working release-coordination surface that detects git-flow drift and lets the user fix it per-MR. **31/31 committed requirements delivered (DRIFT-09 and MERGE-03 intentionally descoped by the user); milestone audit `tech_debt`, 0 blockers.**

**Delivered features:**
- **Release branch & milestone awareness + creation** — `release/<version>` and the matching GitLab milestone are resolved and their existence surfaced (six `BranchState` variants in the sidebar, drift triangles on the Releases list); either can be created behind a confirm dialog, with git-ref validation, the real `X.Y.Z (DD.MM.YYYY)` title format, and duplicate-title blocking.
- **Three-channel drift detection** — MRs are discovered via Jira-key linkage, GitLab milestone, and release-branch target, unioned with per-MR provenance; wrong target branch, missing milestone and task-not-in-fix-version are flagged, with merged/closed/draft classified separately and never inflating the count. All fetchers fully paginated.
- **Per-MR corrective actions** — retarget and assign-milestone applied directly from the row, one optimistic mutation per (MR, action) with prefix-scoped rollback, sticky failures and independent retry; no confirm dialog and no warning, by explicit user decision.
- **Post-release merge-back verification** — an advisory-only verdict over four fail-closed evidence channels (tracking MR, `repository/compare` content diff, tags, default branch), preferring MR state and falling back to content comparison; never blocking, no override control.
- **Unified release detail table + epics redesign** — the Issues and MR-Drift tables collapsed into one task-keyed table with inline drift actions and a secondary uncovered-MR table; `/epics` rebuilt as a single-line portfolio row with progressive progress/points enrichment.

**Descoped/reversed during the milestone:** DRIFT-09 (Phase 89 UAT); MERGE-03 manual override (D-12, given twice); Phase 91.1's stacked per-MR sub-lines — built, shown on real data, then reversed to one line per task at live UAT; Phase 91.2's quick-search (D-18), labels (D-08) and flagged-child counts (D-12).

**Known gap:** no GitLab write path (create-branch, create-milestone) has ever executed against a real GitLab instance — those checkpoints were explicitly waived; coverage is mocked-fetch unit tests only.

**Previous milestone shipped:** v1.13 Personal Workspace — 6 phases (81-86), 23 plans, 411 commits, shipped 2026-06-16 (charting foundation, My Tasks command center, graph-driven dashboard redesign).
...
- ✓ `ReleaseDetailPage.tsx` (1518 LOC) decomposed into a 13-file `release-detail/` folder mirroring `issue-detail/`, page shell at 322 LOC, zero behavior change — v1.14 Phase 87
- ✓ Release branch resolved as `release/<version>` (version-only, corrected from milestone-title by D-09); existence surfaced in the sidebar and as Releases-list drift triangles — v1.14 Phase 88
- ✓ Create the missing release branch off the GitLab project's fetched `default_branch`, behind a confirm dialog with git-ref validation — v1.14 Phase 88
- ✓ Create a missing GitLab milestone (format `X.Y.Z (DD.MM.YYYY)`, corrected from `1.1.0` by D-01) behind a confirm dialog, with a recent-milestone reference list and duplicate-title blocking — v1.14 Phase 88
- ✓ Three-channel MR discovery (Jira-key linkage, milestone-carrying, release-branch-targeting) unioned with per-MR provenance; all fetchers fully paginated — v1.14 Phase 89
- ✓ Drift flagging: wrong target branch, missing milestone, task not in fix version; merged/closed/draft classified separately and excluded from the count — v1.14 Phase 89
- ✓ Aggregate per-release drift count on the Releases list from one project-wide open-MR fetch — v1.14 Phase 89
- ✓ Per-MR corrective actions (retarget to release branch, assign release milestone), one optimistic mutation per (MR, action) with prefix-scoped rollback, sticky failure and independent retry — v1.14 Phase 90
- ✓ Post-release merge-back check: advisory verdict over four fail-closed evidence channels, MR-state-preferred with `repository/compare` fallback, never blocking — v1.14 Phase 91
- ✓ One unified task-keyed release detail table with inline BR/MS drift actions, click-to-filter warning badges, and a secondary table for uncovered MRs — v1.14 Phase 91.1
- ✓ `/epics` as a single-line portfolio row (creation order, three-segment progress bar, `done/total SP`, priority, child-status breakdown) with fail-closed enrichment streaming behind first paint — v1.14 Phase 91.2

### Active

<!-- No active milestone. Run /gsd-new-milestone to define v1.15. -->
...
**v1.14 shipped 2026-08-13** — 7 phases (87-91.2, two inserted), 49 plans, 551 commits, 525 files changed (+68,011/−5,675 lines; `taskflow/src` alone 152 files, +21,865/−4,131). The Releases view became a working release-coordination surface: `ReleaseDetailPage.tsx` was first decomposed 1518 → 322 LOC into a 13-file `release-detail/` folder, then grew release-branch and GitLab-milestone existence detection with confirm-gated creation, three-channel MR discovery with drift flagging, per-MR retarget/assign-milestone corrective actions, and an advisory post-release merge-back verdict. Two inserted phases followed from live UAT: 91.1 collapsed the Issues and MR-Drift tables into one task-keyed table (reversing its own specified sub-line design after seeing it on real data), and 91.2 rebuilt `/epics` as a single-line portfolio row. 31/31 committed requirements satisfied; DRIFT-09 and MERGE-03 intentionally descoped by the user. Milestone audit `tech_debt`, 0 blockers. Run `release.sh` to cut the v1.14.x release.

**Carried into the next milestone.** The material gap is live-GitLab evidence: **no write path — create-branch, create-milestone — has ever executed against a real instance.** Those Phase 88 checkpoints were explicitly waived by the user and coverage is mocked-fetch unit tests only; the Phase 90 D-16 approvals/protected-branch probe likewise never ran (no reachable PAT, and per D-16 the outcome changes no UI). Phase 88/90/91.1 verification all sit at `human_needed`. Smaller items: Phase 89 WR-04 (a failed channel query still reads as "no drift" at the channel level, only partially superseded by 91.1's drift-partial banner), Phase 89 WR-06 (list and detail pages tie-break "matched milestone" differently inside the ±7d window), a 4-error biome baseline pre-existing from Phases 81/82, and the accepted tension that `brFlaggedCount` counts distinct flagged MRs while the table renders one row per task.

**Two data findings from the live probe** (`88-RESEARCH.md`) still shape this area: the team writes milestone dates both zero-padded and unpadded, weakening whole-title duplicate detection; and versions `28.9.0`/`33.6.0` each appear twice with different dates, making version-only branch derivation ambiguous. Channel A is bounded by an `updated_after` window derived from unreleased fix versions (measured 4189 MRs / 42 pages / ~15MB unbounded, ~7.8s → ~1.2s windowed — the GitLab instance is throughput-limited, so parallelism alone did not help); Channels B and C stay unbounded, so milestone- or branch-attached MRs are found at any age.

<details>
<summary>Earlier milestones (v1.10 – v1.13)</summary>
...
- **Shipped v1.14:** 2026-08-13 — 7 phases (87-91.2), 49 plans, 551 commits, 525 files changed (+68,011/−5,675 lines)
...
| EpicsPage renders fetchEpicsBasic first, then merges fetchEpicEnrichmentMap progressively | Phase 91.2 (roadmap D-04) accepted the second batched query on the condition it never blocks first paint — one batched JQL for all epics, not N per-epic queries, which is what the original decision was avoiding | ✓ Good — supersedes the earlier "enrichment too slow for list view" decision (Phase 91.2) |
...
| `release/<version>` derived from the version number only, not the milestone title (v1.14 Phase 88, D-09) | Milestone titles carry a date suffix (`33.5.0 (21.07.2026)`); the branch must be `release/33.5.0` | ✓ Good — matches the team's real branches; ambiguity only where a version number is reused across two Jira versions |
| Default branch read from the GitLab API, never configured (v1.14 Phase 88) | The user confirmed `develop` *is* the project default branch — configuration would drift from reality | ✓ Good — one less setting, correct per-project |
| Confirm dialog on create actions only; retarget/assign apply directly (v1.14 Phases 88/90, D-16) | Creating team-visible refs warrants a gate; retarget/assign are reversible enough. The user was shown GitLab's documented approval-reset behavior and chose the direct action anyway | ✓ Good — user-decided twice; no warning, no dialog on the fix actions |
| Two independent mutations per MR, not one combined PUT (v1.14 Phase 90) | `PUT /merge_requests/:iid` accepts both fields in one call, but a combined call cannot express "assign succeeded, retarget failed" as two retryable states | ✓ Good — MRFIX-03 independence holds; a field-scoped inverse patch replaced whole-array rollback after CR-01 |
| Channel C (and every new MR fetcher) must be fully paginated (v1.14 Phase 89) | `fetchRecentProjectMRs` was capped at 100; the fetch-once page-cap bug class has already hit this codebase twice | ✓ Good — capped fetcher and its `buildWrongMilestoneMap` heuristic deleted outright |
| Channel A bounded by an `updated_after` window; B and C unbounded (v1.14 Phase 89) | Unbounded Channel A measured 4189 MRs / 42 pages / ~15MB / ~7.8s; the instance is throughput-limited so parallelism didn't help | ⚠️ Revisit — ~1.2s windowed, but a very old Jira-linked MR with no milestone and no release-branch target can fall outside the window |
| Merge-back detection is layered, advisory, and diff-based (v1.14 Phase 91, D-04/D-12) | The branch `merged` field false-negatives on squash/rebase; the team merges back both via MR and via direct push. Diff-based rather than commit-based so a merge-method change can't silently start false-negativing | ✓ Good — four fail-closed evidence channels; no override control per D-12, MERGE-03 descoped |
| Every drift/verification channel fails closed, never to an empty result (v1.14 Phases 89/91/91.1/91.2) | An `?? []` or `.catch(() => [])` renders a failed fetch as a *verified* negative — the same absence-of-evidence bug shipped in 88-CR-01, 91-07, 91.1-CR-06 and 91.2-D-15 | ✓ Good — recurring bug class now has a named rule; three gap-closure rounds in Phase 91 exist because of it |
| Phase 91.1 reversed its own spec at live UAT: one line per task, not stacked MR sub-lines | Built as specified, then rejected on real data — "99% of the time there is 1 MR for 1 task". Most-relevant MR shown (flagged > evaluated > highest iid) with a `+N` marker; no MR dropped from the data layer | ✓ Good — roadmap criteria struck through and replaced rather than silently diverged from |
| `/epics` rows are div+flex, not `<table>` (v1.14 Phase 91.2, D-07) | Narrow columns collapse to zero width in the Tauri/WebKit webview; extending the existing `<colgroup>` would re-trigger a documented recurring bug | ✓ Good — verified at all three densities in the real webview |
| `fetchEpicsBasic` ordering is a per-caller parameter, in the query key (v1.14 Phase 91.2) | The fetcher is shared with the Sidebar, Backlog and Sprint Board pickers under one cache key — this phase's `created ASC` was silently reordering all of them | ✓ Good — defaults to the original `updated DESC`; the two orderings cannot serve each other's cache |

---
*Last updated: 2026-08-13 — after the v1.14 Release Management milestone*
