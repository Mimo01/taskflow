...
**v1.14 in progress** — Phase 88 (Release Branch & Milestone Creation) complete 2026-08-10, 11 plans. Delivered the release-branch existence signal and both write paths: `resolveBranchState` (blocked → unresolvable → invalid-ref → check-failed → loading → exists/released/missing), create-branch off the project's fetched `default_branch`, and create-milestone with client-side duplicate detection. RELBR-01..05 and RELMS-01..04 satisfied at code level.

Three gap-closure rounds ran after the initial code review found blockers: 88-07..88-10 closed CR-01/CR-02/CR-03 (absence-of-evidence drift indicators, milestone cache-invalidation granularity, indistinguishable failed-vs-loading branch check) plus WR-01/03/04/06/10/11.

Live-GitLab evidence is partial by explicit user decision. 88-11 Tasks 2–4 (live create-branch, live create-milestone propagation, restricted-PAT error surfacing) were **waived**; those write paths have still never executed against a real instance. Task 5 ran: `probe.sh` against the live instance confirmed `project_id` is present (D-07 filter viable, zero inherited milestones) and found zero title collisions across 265 real milestones, resolving RESEARCH Open Question #1. Three items persist in `88-HUMAN-UAT.md`.

UAT feedback then reshaped the surface: released versions no longer report a missing release branch (branches are deleted on merge — the live probe found only `release/33.7.0` surviving), the sidebar shows a `released` state with the `v<version>` tag when one exists, the Releases-list milestone warning and "No GitLab link" label were merged into one indicator, a green branch icon marks a present branch, and the create-milestone dialog now takes the version only with the date appended from Jira.

Carried forward: WR-01 in `gitlab.ts` — GitLab's object-keyed validation-error body still renders as `[object Object]` in both create dialogs. Two data findings recorded in `88-RESEARCH.md`: the team writes milestone dates both padded and unpadded (weakening whole-title duplicate detection), and versions `28.9.0`/`33.6.0` each appear twice with different dates (making version-only branch derivation ambiguous).
...
**Phase 90 complete (2026-08-11)** — Flagged drift is now fixable in place. The BR and MS drift cells became conditionally interactive inside the existing 28px row footprint: hovering a flagged row swaps `⚠` for an action icon, and a click retargets the MR to the release branch or assigns the release milestone. Each (MR, action) pair owns its own `useMrFixMutation` instance with an optimistic multi-cache patch, field-scoped rollback, and a sticky local failure state that survives background refetches. `gitlab.ts` gained its first MR-mutating endpoint, `updateMergeRequest` — restricted to `target_branch`/`milestone_id` by explicit per-key body assembly so no caller-supplied field can reach GitLab — plus `flattenGitLabError`, which finally kills the `[object Object]` validation-error rendering carried forward from Phase 88 (WR-01) for this write path. Per user decision D-16 there is no confirm dialog and no inline warning on retarget, regardless of approval-reset behavior. Validated in Phase 90: MRFIX-01..MRFIX-04.
...
**Phase 91.2 complete (2026-08-12)** — Inserted phase. `/epics` is now a portfolio surface rather than a five-column list: one div+flex line per epic carrying key · priority · name · status · progress · points · assignee, ordered oldest-first, with a three-segment done/in-progress/todo bar and `done/total SP`. Progress and points stream in behind first paint from a second batched enrichment query (one JQL for all epics), so the deliberate fast-first-paint behaviour is preserved. Verification passed 11/11; EPIC-01..07 satisfied. The dead `services/jira/epics.ts` duplicate and the drifted `fetchEpicsWithEnrichment` were both deleted.

Human UAT drove three changes: priority moved between key and name to match `BacklogRow`, and enrichment pages now fetch in parallel (bounded to 6) after the bars took too long to appear — `fetchAllSearchPagesConcurrent` steps by the server-reported `maxResults`, not the requested page size, and fails closed rather than returning a silently short result.

Code review then found two real blockers, both fixed and regression-tested: a stale-banner retry pinned the skeleton forever because `isRefreshing` was reset by an effect keyed on `isLoading`, which never transitions once data exists (CR-01); and a failed `readSecret` left every progress cell shimmering indefinitely, because a disabled enrichment query read as *pending* rather than as an error (CR-02). A third, found while verifying those, was that cached rows were gated on `!isError` — so a failed refetch blanked the page while `StaleDataBanner` sat above it claiming the cached data was still visible. A note on method: the first regression test written for CR-01 passed against the unfixed code; the bug reproduces on the cached-data path, not the no-data path the review described. Each fix here was checked to fail before it passed.

One decision worth recording: `fetchEpicsBasic` is shared with the Sidebar, Backlog and Sprint Board epic pickers under a single cache key, so this phase's `created ASC` ordering (D-17) was silently reordering all of them. Ordering is now a per-caller parameter defaulting to the original `updated DESC`, with the value in the query key so the two orderings cannot serve each other's cache.
...
---
*Last updated: 2026-08-12 — after Phase 91.2 (Epics Page Redesign) completed*
