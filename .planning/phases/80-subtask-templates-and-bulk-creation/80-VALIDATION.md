...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| W0 | 01 | 0 | SUBTPL-02 | T-80-01 | N/A | unit | `npm run test -- src/stores/subtask-templates.store.test.ts` | ✅ | ✅ green |
| W0 | 01 | 0 | SUBTPL-03 / SUBTPL-05 | — | N/A | unit | `npm run test -- src/routes/dashboard/resolveTemplateFields.test.ts` | ✅ | ✅ green |
| W0 | 01 | 0 | SUBTPL-08 | — | N/A | unit | `npm run test -- src/routes/dashboard/resolveRowPlaceholders.test.ts` | ✅ | ✅ green |
| W0 | 01 / 04 | 0 | SUBTPL-06 / SUBTPL-07 | T-80-08 | N/A | unit | `npm run test -- src/routes/dashboard/BulkCreateSubtasksModal.test.ts` | ✅ | ✅ green |
...
- [x] `taskflow/src/stores/subtask-templates.store.test.ts` — covers SUBTPL-02 (store add/remove/rename/move persistence)
- [x] `taskflow/src/routes/dashboard/resolveTemplateFields.test.ts` — covers SUBTPL-03, SUBTPL-05 (field-drop computation; pure function)
- [x] `taskflow/src/routes/dashboard/resolveRowPlaceholders.test.ts` — covers SUBTPL-08 (placeholder resolver; pure function extractable from modal)
- [x] `taskflow/src/routes/dashboard/BulkCreateSubtasksModal.test.ts` — covers SUBTPL-06, SUBTPL-07 (retry loop, sequential ordering)
...
**Approval:** validated 2026-06-07 — all 4 Wave-0 test files green (46 tests)
...
All four Wave-0 unit test files exist and run green (46 tests passing in 888ms). The draft VALIDATION.md predated execution and listed them as `❌ pending`; this audit confirmed every automated requirement (SUBTPL-02, -03, -05, -06, -07, -08) is COVERED. Manual-only verifications (SUBTPL-01, -04, plus live-network aspects of -06/-07) remain manual by design and are unchanged.
