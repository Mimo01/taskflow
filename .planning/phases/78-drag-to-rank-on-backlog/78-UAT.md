...
- truth: "After a drag that auto-scrolled the page, releasing returns the page to a consistent state — subsequent clicks select the row actually under the pointer"
  ...
  fix_attempts: "1) commit 4f0cfdd3 portal overlay + droppable MeasuringStrategy.Always (fixed during-drag clone tracking, not the post-scroll-drop desync). 2) commit f4ecf9d9 forced drag-end reconcile re-render (insufficient). 3) commit 738e1c2c single scroll container (outer overflow-hidden) — still one-row drift. 4) commit 9fd20297 autoScroll={false} fully fixed it but user wanted autoscroll kept (later reverted). FINAL — quick task 260604-knq commit 2a2a4c30: removed DragOverlay + createPortal, drag the real row in place (opacity 0.85 + shadow-lg ring-2 ring-primary), autoScroll kept ON; dnd-kit's non-overlay path compensates scroll delta. npm run check clean; vitest dashboard+jira 810 passed. Awaiting human re-verification."
