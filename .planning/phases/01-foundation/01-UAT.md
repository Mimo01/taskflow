...
number: 2
name: Onboarding Wizard: Full Flow — retry after fixes
expected: |
  Both issues from the first attempt are now fixed: selector width (w-full added) and GitLab TLS (danger.acceptInvalidCerts). Retry: complete all 5 wizard steps end-to-end and land on Dashboard.
...
result: issue
reported: "the project selector is too narrow / Cannot reach https://git.devel.sun.orange.sk — check the base URL / In network: error sending request for url"
severity: major
