...
- [x] **Phase 3: Notifications Hub** - Unified notification feed with polling, OS desktop notifications, in-app badge, and read/unread tracking (completed 2026-03-12)
...
- [x] 01-06-PLAN.md — Verification gap closure: fix 9 failing unit tests in jira.test.ts and gitlab.test.ts by replacing vi.stubGlobal with vi.mock for @tauri-apps/plugin-http named fetch
...
- [x] 02-05-PLAN.md — UAT gap closure: delete postcss.config.js + dead tailwind.config.js files that strip all Tailwind CSS from the build output
- [x] 02-06-PLAN.md — UAT gap closure: wire StatusPopover and InlineComment into TaskRow/MyTasksTab (replace no-op stubs with real state and mutations)
- [x] 02-07-PLAN.md — UAT gap closure: fix TokenSection silent-failure pattern — GitLab group and Jira project selectors now show loading/error states instead of silently hiding
...
| 3. Notifications Hub | 2/2 | Complete    | 2026-03-12 |
