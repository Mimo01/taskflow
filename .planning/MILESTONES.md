...
## v1.14 Release Management (Shipped: 2026-08-13)

**Phases completed:** 7 phases (87-91.2, incl. 2 inserted), 49 plans, 121 tasks
**Timeline:** 2026-06-16 → 2026-08-13 (phase execution 2026-08-10 → 2026-08-12)
**Codebase:** 525 files changed (+68,011/−5,675 lines), 551 commits; `taskflow/src` alone 152 files (+21,865/−4,131)
**Git range:** 8f848976 (chore: bump version to 1.13.0) → d49d9deb (docs(quick-260813-epk): make all tests and linters pass)

**Key accomplishments:**

1. **Release detail decomposition (Phase 87)** — `ReleaseDetailPage.tsx` (1518 LOC) split into a 13-file `release-detail/` folder mirroring the `issue-detail/` precedent: a React-free `releaseSummaries.ts`, a `useReleaseDetail.ts` hook owning all 6 queries and every derived value, a `useEditRelease.ts` state hook, and one file per section (`ReleaseHeader`, `DescriptionsSection`, `LabelSummarySection`, `IssuesSection`, `ReleaseDetailSidebar`, `EditReleaseModal`, `MetaRow`, skeleton). Page shell 1518 → 322 lines, zero user-visible change.
2. **Release branch & GitLab milestone creation (Phase 88)** — the release view now resolves `release/<version>` (version-only derivation per D-09) and its matching GitLab milestone, shows six `BranchState` variants in the sidebar plus per-row drift triangles on the Releases list (one paginated `search=release/` fetch), and can create either behind a confirm dialog: git-ref validation, the real `X.Y.Z (DD.MM.YYYY)` title format, a read-only recent-milestone reference list, and duplicate-title blocking. Five new `gitlab.ts` functions (`fetchProject`, `fetchProjectBranches`, `fetchBranch` with 404-as-missing, `createBranch`, `createMilestone`), all via `apiFetch('gitlab', ...)`. A gap-closure wave added `check-failed` states, project-granular invalidation, body-first 401/403 error text, and in-flight dismissal locks.
3. **Three-channel drift detection (Phase 89)** — MRs relevant to a release are discovered three ways — (A) Jira-key linkage to fix-version issues, (B) carrying the GitLab milestone, (C) targeting the release branch — and unioned in `driftDetection.ts` with per-MR channel provenance. Three predicates flag wrong target branch, missing milestone, and task-not-in-fix-version; merged/closed/draft MRs are classified separately and never inflate the count. All three fetchers are fully paginated (the capped `fetchRecentProjectMRs` and the `buildWrongMilestoneMap` heuristic were deleted outright). The Releases list gained an aggregate `{n} drift` badge from one project-wide open-MR fetch.
4. **Per-MR corrective actions (Phase 90)** — `updateMergeRequest` plus a shared `flattenGitLabError` normaliser (string / string[] / field-keyed Grape object) gave the app its GitLab write path; `useMrFixMutation` runs one mutation instance per (MR, action) cell with a prefix-matched optimistic patch across all three channel caches, rollback, and a sticky failure that survives background refetch. `DriftActionCell` renders BR/MS as focus-reachable hover-revealed buttons in the same 28px footprint, with a `useRef`-frozen row order so a settling cell never jumps under the pointer. Retarget disables itself while the release branch doesn't exist. Approved by the developer against live GitLab data.
5. **Post-release merge-back verification (Phase 91)** — an advisory-only verdict (no override control, MERGE-03 descoped by D-12) answering whether `release/<tag>` landed in the project default branch. `resolveMergeBackVerdict` is an eleven-step precedence resolver over four evidence channels — tracking MR (`fetchSourceBranchMRs`, target-branch-filtered, deterministic pick), content diff (`compareRefs`, diff-based per D-04 so a merge-method change can't silently false-negative), tags, and default branch — every one of which now **fails closed**: three rounds of verification gap-closure replaced swallowed errors and permanent-loading states with terminal `couldn't verify` outcomes, ending with a `TagChannelHealth` discriminant that stops the Release Branch row asserting "No matching tag found" while the lookup is pending.
6. **Unified release detail task table (Phase 91.1, INSERTED)** — the separate Issues and MR-Drift tables were replaced by one `UnifiedTaskTable.tsx` keyed on Jira tasks, with BR/MS drift marks and their click-to-fix actions re-hosted inline and a secondary table for MRs the release's tasks don't cover (no-key and out-of-scope-key, the latter named and flagged). Two clickable `⚠ N wrong branch` / `⚠ N no milestone` work-queue badges filter the table. Notably, the specified stacked MR sub-lines were built, shown on real data, and then **reversed by the developer at live UAT** — the shipped table is one line per task, most-relevant MR displayed (flagged wins, then evaluated, tie-break highest iid) with a hoverable `+N` marker; no MR is dropped from the data layer.
7. **Epics page redesign (Phase 91.2, INSERTED)** — `/epics` moved from a five-column `updated DESC` list to a div+flex single-line row (key · name · status · priority · progress · points · assignee) ordered `created ASC`, with a three-segment Done/In-Progress/To-Do bar and `done/total SP` streaming in from a second `jira-epics-enrichment` query behind first paint. Enrichment fails **closed** (D-15) rather than rendering a failed fetch as "0 stories". div+flex rather than `<table>` deliberately, to avoid the documented Tauri/WebKit zero-width-column collapse. Quick-search (D-18), labels (D-08) and flagged-child counts (D-12) were all specified and then cut by the user.

Plus a substantial quick-task layer between v1.13 and v1.14 (~25 tasks): compactness/font-size settings and their coverage sweep, search-modal and recently-visited type icons, release-detail layout moves, worklog and issue-detail sidebar additions, the `gitlab.ts` object-keyed error flattening, Jira field-validation error flattening, and a full tests/linters green pass.

**Known gaps and deferred items:** 111 surfaced by the open-artifact audit (see STATE.md Deferred Items) — overwhelmingly historical noise: 95 stale quick-task dirs (dating to 260521), 7 pre-v1.14 debug sessions, 1 deferred priority-stripe-by-REST-rank TODO. The genuinely v1.14-scoped items are the Phase 88 and Phase 90 human-verification gaps: **no GitLab write path (create-branch, create-milestone) has ever executed against a real GitLab instance** — the checkpoints were explicitly waived by the user and coverage is mocked-fetch unit tests only — and the Phase 90 D-16 approvals/protected-branch probe was never run (no reachable PAT; per D-16 this changes no UI either way). The milestone audit closed at `tech_debt`: 31/31 requirements (DRIFT-09 and MERGE-03 intentionally descoped by the user), 7/7 phases verified, 10/10 integration paths, 3/3 flows.
