...
Last activity: 2026-06-06 - Completed quick task 260606-rgc: epics page brought to backlog parity (headerless table + colgroup width preservation, always-render Unassigned CachedAvatar matching BacklogRow, canonical status pill confirmed) — verified 5/5, npm run check GREEN
...
| 260606-rgc | Epics page brought to backlog parity (EpicsPage.tsx) — removed the `<thead>` column-header row (headerless like the backlog table); added a 5-`<col>` `<colgroup>` (w-1 / flex / w-28 / w-32 / w-12) to preserve column widths that previously lived only on the deleted `<th>`s; assignee cell now ALWAYS renders CachedAvatar with `name={...|| 'Unassigned'}` (distinct dashed-border unassigned treatment) matching BacklogRow, replacing the old `assignee ? … : null` conditional; status pill confirmed already canonical (bare statusPillClass span, no edit); targeted scope (color bar + epic-name badge + key cells kept; no issue-type/priority/points added); --full pipeline (discuss+research+check+review+verify), verified 5/5, npm run check GREEN | 2026-06-06 | 5da13469 | Verified | [260606-rgc-make-the-epics-page-match-more-backlog-v](./quick/260606-rgc-make-the-epics-page-match-more-backlog-v/) |
