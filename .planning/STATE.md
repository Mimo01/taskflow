...
| 260606-ugr | Subtask parent-issue link redesigned from a faint muted breadcrumb (`↗ KEY — summary`) into a prominent clickable parent card above the title — full-width `<button>` with elevated surface (`border bg-muted/50 hover:bg-muted`, rounded, `px-3 py-2`), parent IssueTypeIcon (conditional), uppercase "Parent" label, mono key, foreground truncating summary (`min-w-0 flex-1 truncate pr-0.5`), correctly-colored status pill (color from `statusCategory.key`, text = `status.name`, wrapped in a `flex` parent per the statuspill-flex memory — NOT keyed on the name), and a trailing `ArrowUpRight`; `aria-label` added. One shared component (`IssueDetailContent.tsx`) drives BOTH the full page (IssueDetailView two-column) and the peek panel (PeekPanel → single-column), so the single edit covers both. Required widening the CONSUMED `JiraIssueDetail.fields.parent` type at `jira.ts:1239` (dual-file gotcha — NOT jira/types.ts) to add optional `issuetype` + `status.statusCategory.key`; data already arrives from `fetchIssueDetail`'s `parent` field (Jira DC returns nested issuetype/status). Existing `onOpenIssue(parent.key)` navigation preserved (peek breadcrumb fix 943cba44 intact). --full pipeline (discuss+research+check×2+review+verify); plan-checker caught 2 type-site blockers + 1 status-pill blocker across 2 revisions; code review 0 blocker/0 warning; verified 8/8; npm run check GREEN (465 files), no new `any`. | 2026-06-06 | 42926b02 | Verified | [260606-ugr-on-subtasks-redesign-how-the-parent-issu](./quick/260606-ugr-on-subtasks-redesign-how-the-parent-issu/) |
