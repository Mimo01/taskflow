...
Last activity: 2026-06-06 - Quick task 260606-rgc APPROVED & closed: epics page brought to backlog parity (headerless table + colgroup, always-render Unassigned CachedAvatar). UAT follow-ups fixed: status pill wrapped in flex div so statusPillClass min-w/text-center apply (needs flex-item parent), status col tightened w-32→w-28, assignee right padding px-3→pr-6 + col w-16 so the avatar isn't flush to the edge. npm run check GREEN
