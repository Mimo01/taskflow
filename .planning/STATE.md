...
Last activity: 2026-06-06 - Quick task 260606-s09 APPROVED & closed: filter stories by "Unassigned" in Backlog + Sprint Board, merged into the assignee filter via a new shared assignee-filter helper (reserved sentinel, collision-proofed, pinned to top, shown only when an unassigned issue is visible, both view predicates unified). UI polish: Unassigned option rendered italic + muted with a divider below it (generic distinctValue prop on FilterDropdown), and pr-0.5 so the italic trailing glyph isn't clipped by truncate. --full pipeline; verified 6/6; 12/12 tests; npm run check GREEN.
