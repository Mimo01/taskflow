...
Last activity: 2026-06-06 - Completed quick task 260606-spj: issue peek redesign — header now shows issue-type icon + key + truncated title (controls stay visible during load, title via a deduped same-key useQuery so no extra fetch), and the Merge Requests section moved out of the top sidebar to the bottom of the single-column peek content (new useLinkedMRs hook + omitMergeRequests prop; two-column full page unchanged). --full pipeline; verified 6/6; 7/7 PeekPanel tests; npm run check GREEN.
